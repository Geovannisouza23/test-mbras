#!/bin/bash
# scripts/check_debug_prints.sh
# Falha se encontrar prints de debug proibidos no código Go

set -e

if grep -rE 'fmt\.Print|log\.Print|println|print\(' --exclude-dir=.git --exclude-dir=vendor --exclude='scripts/check_debug_prints.sh' .; then
